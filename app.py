...
app.secret_key = os.environ.get(
    'SECRET_KEY', 'your_super_secret_key_here_please_change_this_to_a_complex_random_string')
...
def get_cached_wheels(query, brand_filter):
    print(
        f"--- CACHE MISS (WHEELS) --- Fetching wheels from DB for query='{query}', brand='{brand_filter}'")
    conn = get_db()
    return database.get_all_wheels(conn, query=query, brand_filter=brand_filter, include_deleted=False)


# ข้อมูลยี่ห้อเปลี่ยนแปลงไม่บ่อย Cache ไว้นานขึ้นได้
...
class User(UserMixin):
    ...
    def get(conn, user_id):
        ...
        if "psycopg2" in str(type(conn)):
            ...
            user_data = conn.execute(
                "SELECT id, username, password, role FROM users WHERE id = ?", (user_id,)).fetchone()
        if user_data:
    ...
    def get_by_username(conn, username):
        ...
        if "psycopg2" in str(type(conn)):
            ...
            user_data = conn.execute(
                "SELECT id, username, password, role FROM users WHERE username = ?", (username,)).fetchone()
        if user_data:
            return User(user_data['id'], user_data['username'], user_data['password'], user_data['role'])
        return None
...
def process_tire_report_data(all_tires, current_user_obj, include_summary_in_output=True):
    ...
    sorted_tires = sorted(all_tires, key=lambda x: (
        x['brand'], x['model'], x['size']))
    ...
    for tire in sorted_tires:
        ...
        tire_dict['display_price_for_4'] = original_price_per_item * \
            4 if original_price_per_item is not None else None
        ...
        grouped_data[brand]['items_list'].append({
            ...
            # Original price_per_item
            'price_per_item': tire_dict['price_per_item'],
            ...
            # This will be the promo price for retail_sales if active, else regular
            'display_price_for_4': tire_dict['display_price_for_4'],
    ...
    for brand, data in grouped_data.items():
        data['summary'] = {
            ...
            # type: ignore
            'formatted_quantity': f'<span class="summary-quantity-value">{brand_quantities[brand]}</span>'
        }
    return grouped_data
...
def process_wheel_report_data(all_wheels, include_summary_in_output=True):
    ...
    sorted_wheels = sorted(all_wheels, key=lambda x: (
        x['brand'], x['model'], x['diameter'], x['width'], x['pcd']))
...
def index():
    ...
    is_tire_search_active = bool(tire_query or (
        tire_selected_brand and tire_selected_brand != 'all'))
    ...
    wheel_selected_brand = request.args.get(
        'wheel_brand_filter', 'all').strip()
    is_wheel_search_active = bool(wheel_query or (
        wheel_selected_brand and wheel_selected_brand != 'all'))
    ...
    wheels_by_brand_for_display = process_wheel_report_data(
        wheels_for_display, include_summary_in_output=is_wheel_search_active)
        ...
        if not name or not promo_type or not value1:
            ...
            try:
                ...
                if promo_type == 'buy_x_get_y' and (value2 is None or value1 <= 0 or value2 <= 0):
                    raise ValueError(
                        "สำหรับ 'ซื้อ X แถม Y' โปรดระบุ X และ Y ที่มากกว่า 0")
                ...
                conn = get_db()
                database.add_promotion(
                    conn, name, promo_type, value1, value2, is_active)
    ...
    if request.method == 'POST':
        ...
        if not name or not promo_type or not value1:
            ...
            try:
                ...
                conn = get_db()
                database.update_promotion(
                    conn, promo_id, name, promo_type, value1, value2, is_active)
...
def delete_promotion(promo_id):
    ...
    if promotion is None:
        ...
        try:
            ...
            flash(
                'ลบโปรโมชันสำเร็จ! สินค้าที่เคยใช้โปรโมชันนี้จะถูกตั้งค่าโปรโมชันเป็น "ไม่มี"', 'success')
            cache.delete_memoized(get_all_promotions_cached)
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        if submit_type == 'add_tire':
            ...
            scanned_barcode_for_add = request.form.get(
                'barcode_id_for_add', '').strip()
            ...
            if scanned_barcode_for_add:
                existing_barcode_tire_id = database.get_tire_id_by_barcode(
                    conn, scanned_barcode_for_add)
                existing_barcode_wheel_id = database.get_wheel_id_by_barcode(
                    conn, scanned_barcode_for_add)
                if existing_barcode_tire_id or existing_barcode_wheel_id:
                    flash(
                        f"Barcode ID '{scanned_barcode_for_add}' มีอยู่ในระบบแล้ว. ไม่สามารถใช้ซ้ำได้.", 'danger')
                    active_tab = 'tire'
                    return render_template('add_item.html', form_data=form_data, active_tab=active_tab, current_year=current_year, all_promotions=all_promotions, current_user=current_user)
            ...
            try:
                ...
                cost_sc = float(
                    cost_sc) if cost_sc and cost_sc.strip() else None
                cost_dunlop = float(
                    cost_dunlop) if cost_dunlop and cost_dunlop.strip() else None
                cost_online = float(
                    cost_online) if cost_online and cost_online.strip() else None
                wholesale_price1 = float(
                    wholesale_price1) if wholesale_price1 and wholesale_price1.strip() else None
                wholesale_price2 = float(
                    wholesale_price2) if wholesale_price2 and wholesale_price2.strip() else None

                year_of_manufacture = year_of_manufacture.strip(
                ) if year_of_manufacture and year_of_manufacture.strip() else None

                with conn.cursor() as cursor:
                    if "psycopg2" in str(type(conn)):
                        cursor.execute(
                            "SELECT id FROM tires WHERE brand = %s AND model = %s AND size = %s", (brand, model, size))
                    else:
                        cursor.execute(
                            "SELECT id FROM tires WHERE brand = ? AND model = ? AND size = ?", (brand, model, size))

                existing_tire = cursor.fetchone()

                if existing_tire:
                    flash(
                        f'ยาง {brand.title()} รุ่น {model.title()} เบอร์ {size} มีอยู่ในระบบแล้ว หากต้องการแก้ไข กรุณาไปที่หน้าสต็อก', 'warning')
                else:
                    ...
                    if scanned_barcode_for_add:
                        database.add_tire_barcode(
                            conn, new_tire_id, scanned_barcode_for_add, is_primary=True)
                    conn.commit()
                    flash(
                        f'เพิ่มยาง {brand.title()} รุ่น {model.title()} เบอร์ {size} จำนวน {quantity} เส้น สำเร็จ!', 'success')
                    cache.clear()
                ...
                if "UNIQUE constraint failed" in str(e) or "duplicate key value violates unique constraint" in str(e):
                    flash(
                        f'เกิดข้อผิดพลาด: ข้อมูลซ้ำซ้อนในระบบ หรือ Barcode ID นี้มีอยู่แล้ว. รายละเอียด: {e}', 'warning')
                else:
            ...
            if image_file and image_file.filename != '':
                if allowed_image_file(image_file.filename):
                    try:
                        ...
                        flash(
                            f'เกิดข้อผิดพลาดในการอัปโหลดรูปภาพไปยังเซิฟเวอร์: {e}', 'danger')
                        active_tab = 'wheel'
                        return render_template('add_item.html', form_data=form_data, active_tab=active_tab, current_year=current_year, all_promotions=all_promotions, current_user=current_user)
                else:
                    flash(
                        'ชนิดไฟล์รูปภาพไม่ถูกต้อง อนุญาตเฉพาะ .png, .jpg, .jpeg, .gif เท่านั้น', 'danger')
                    active_tab = 'wheel'
                    return render_template('add_item.html', form_data=form_data, active_tab=active_tab, current_year=current_year, all_promotions=all_promotions, current_user=current_user)
            ...
            try:
                ...
                if existing_wheel:
                    flash(
                        f'แม็ก {brand.title()} ลาย {model.title()} ขนาด {diameter}x{width} มีอยู่ในระบบแล้ว', 'warning')
                else:
                    ...
                    if scanned_barcode_for_add:
                        database.add_wheel_barcode(
                            conn, new_wheel_id, scanned_barcode_for_add, is_primary=True)
                    conn.commit()
                    flash(
                        f'เพิ่มแม็ก {brand.title()} ลาย {model.title()} จำนวน {quantity} วง สำเร็จ!', 'success')
                    cache.clear()
    ...
    if request.method == 'POST':
        ...
        if not brand or not model or not size or not str(price_per_item):
            ...
            try:
                ...
                cost_online = float(
                    cost_online) if cost_online and cost_online.strip() else None
                wholesale_price1 = float(wholesale_price1) if pd.notna(
                    wholesale_price1) and wholesale_price1.strip() else None
                wholesale_price2 = float(wholesale_price2) if pd.notna(
                    wholesale_price2) and wholesale_price2.strip() else None
                ...
                if "UNIQUE constraint failed" in str(e) or "duplicate key value violates unique constraint" in str(e):
                    flash(
                        f'ยางยี่ห้อ {brand} รุ่น {model} เบอร์ {size} นี้มีอยู่ในระบบแล้วภายใต้ ID อื่น โปรดตรวจสอบ', 'warning')
                else:
...
# <--- เพิ่ม 'GET' เข้าไป
@app.route('/api/tire/<int:tire_id>/barcodes', methods=['GET', 'POST', 'DELETE'])
@login_required
def api_manage_tire_barcodes(tire_id):
    ...
    try:
        if request.method == 'POST':
            ...
            existing_tire_id_by_barcode = database.get_tire_id_by_barcode(
                conn, barcode_string)
            # ... (ที่เหลือเหมือนเดิม) ...
            database.add_tire_barcode(
                conn, tire_id, barcode_string, is_primary=False)
            conn.commit()
    ...
    if request.method == 'POST':
        ...
        if not brand or not model or not pcd or not str(diameter) or not str(width) or not str(retail_price):
            ...
            try:
                ...
                wholesale_price1 = float(
                    wholesale_price1) if wholesale_price1 else None
                wholesale_price2 = float(
                    wholesale_price2) if wholesale_price2 else None
                cost = float(cost) if cost and cost.strip() else None
                            ...
                            if current_image_url and "res.cloudinary.com" in current_image_url:
                                ...
                                if public_id_match:
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"Error deleting old image from Cloudinary: {e}")

                            current_image_url = new_image_url

                        except Exception as e:
                            flash(
                                f'เกิดข้อผิดพลาดในการอัปโหลดรูปภาพไปยัง Cloudinary: {e}', 'danger')
                ...
                database.update_wheel(conn, wheel_id, brand, model, diameter, pcd, width, et, color,
                                      cost, cost_online, wholesale_price1, wholesale_price2, retail_price, current_image_url)
                ...
                if "UNIQUE constraint failed" in str(e) or "duplicate key value violates unique constraint" in str(e):
                    flash(
                        f'แม็กยี่ห้อ {brand} ลาย {model} ขอบ {diameter} รู {pcd} กว้าง {width} นี้มีอยู่ในระบบแล้วภายใต้ ID อื่น โปรดตรวจสอบ', 'warning')
                else:
...
def api_manage_wheel_barcodes(wheel_id):
    ...
    try:
        if request.method == 'POST':
            existing_tire_id_by_barcode = database.get_tire_id_by_barcode(
                conn, barcode_string)
            existing_wheel_id_by_barcode = database.get_wheel_id_by_barcode(
                conn, barcode_string)
            ...
            database.add_wheel_barcode(
                conn, wheel_id, barcode_string, is_primary=False)
            conn.commit()
            # Use 201 for created
            return jsonify({"success": True, "message": "เพิ่ม Barcode ID สำเร็จ!"}), 201
    ...
    if not brand or not model or not year_start:
        ...
        try:
            ...
            if year_end and year_end < year_start:
                ...
                database.add_wheel_fitment(
                    conn, wheel_id, brand, model, year_start, year_end)
...
def stock_movement():
    ...
    if "psycopg2" in str(type(conn)):
        ...
        tire_movements_history_raw = conn.execute(
            tire_movements_query).fetchall()

    processed_tire_movements_history = []
    for movement in tire_movements_history_raw:
        movement_data = dict(movement)
        movement_data['timestamp'] = database.convert_to_bkk_time(
            movement_data['timestamp'])  # ใช้ database.convert_to_bkk_time
    ...
    if "psycopg2" in str(type(conn)):
        ...
        wheel_movements_history_raw = conn.execute(
            wheel_movements_query).fetchall()
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if bill_image_file and bill_image_file.filename != '':
                if allowed_image_file(bill_image_file.filename):
                    try:
                        ...
                        flash(
                            f'เกิดข้อผิดพลาดในการอัปโหลดรูปภาพบิลไปยัง Cloudinary: {e}', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                else:
                    flash(
                        'ชนิดไฟล์รูปภาพบิลไม่ถูกต้อง อนุญาตเฉพาะ .png, .jpg, .jpeg, .gif เท่านั้น', 'danger')
                    return redirect(url_for('stock_movement', tab=active_tab_on_error))
            ...
            online_platform_id_str = request.form.get(
                'online_platform_id')  # สำหรับ OUT ช่องทาง 'ออนไลน์'
            wholesale_customer_id_str = request.form.get(
                'wholesale_customer_id')  # สำหรับ OUT ช่องทาง 'ค้าส่ง'
            return_customer_type = request.form.get('return_customer_type')

            # NEW: รับค่าสำหรับ 'ชื่อร้านยางที่คืน'
            return_wholesale_customer_id_str = request.form.get(
                'return_wholesale_customer_id')
            # NEW: รับค่าสำหรับ 'แพลตฟอร์มออนไลน์ที่คืน'
            return_online_platform_id_str = request.form.get(
                'return_online_platform_id')
            ...
            channel_name = database.get_sales_channel_name(
                conn, final_channel_id)

            if move_type == 'IN':
                if channel_name != 'ซื้อเข้า':
                    flash(
                        'สำหรับประเภท "รับเข้า" ช่องทางการเคลื่อนไหวต้องเป็น "ซื้อเข้า" เท่านั้น', 'danger')
                    return redirect(url_for('stock_movement', tab=active_tab_on_error))
                ...
                if channel_name != 'รับคืน':
                    flash(
                        'สำหรับประเภท "รับคืน/ตีคืน" ช่องทางการเคลื่อนไหวต้องเป็น "รับคืน" เท่านั้น', 'danger')
                    return redirect(url_for('stock_movement', tab=active_tab_on_error))
                ...
                if return_customer_type == 'ออนไลน์':
                    if not return_online_platform_id_str:  # ใช้ return_online_platform_id_str ที่รับมาใหม่
                        flash(
                            'กรุณาระบุ "แพลตฟอร์มออนไลน์ที่คืน" สำหรับการคืนจาก "ออนไลน์"', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    try:
                        final_online_platform_id = int(
                            return_online_platform_id_str)
                    except ValueError:
                ...
                if return_customer_type == 'หน้าร้านร้านยาง':
                    if not return_wholesale_customer_id_str:  # ตรวจสอบว่ามีค่าส่งมาหรือไม่
                        flash(
                            'กรุณาระบุ "ชื่อร้านยาง" สำหรับการคืนจาก "หน้าร้าน (ร้านยาง)"', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    try:
                        final_wholesale_customer_id = int(
                            return_wholesale_customer_id_str)
                    except ValueError:
                ...
                if channel_name == 'ซื้อเข้า' or channel_name == 'รับคืน':
                    flash(
                        f'สำหรับประเภท "จ่ายออก" ช่องทางการเคลื่อนไหวไม่สามารถเป็น "{channel_name}" ได้', 'danger')
                    return redirect(url_for('stock_movement', tab=active_tab_on_error))

                if channel_name == 'ออนไลน์':
                    if not online_platform_id_str:  # ใช้ online_platform_id_str เดิมสำหรับ OUT ช่องทางออนไลน์
                        flash(
                            'กรุณาระบุ "แพลตฟอร์มออนไลน์" สำหรับช่องทาง "ออนไลน์"', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    try:
                ...
                if channel_name == 'ค้าส่ง':
                    if not wholesale_customer_id_str:  # ใช้ wholesale_customer_id_str เดิมสำหรับ OUT ค้าส่ง
                        flash(
                            'กรุณาระบุ "ชื่อลูกค้าค้าส่ง" สำหรับช่องทาง "ค้าส่ง"', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    try:
                        final_wholesale_customer_id = int(
                            wholesale_customer_id_str)
                    except ValueError:
                        flash('ข้อมูลชื่อลูกค้าค้าส่งไม่ถูกต้อง', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                else:
                    # Make sure this is None for other OUT channels
                    final_wholesale_customer_id = None
            ...
            if submit_type == 'tire_movement':
                ...
                if move_type == 'IN' or move_type == 'RETURN':
                    ...
                    if new_quantity < quantity_change:
                        flash(
                            f'สต็อกยางไม่พอสำหรับการจ่ายออก. มีเพียง {new_quantity} เส้น.', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    new_quantity -= quantity_change
                ...
                flash(
                    f'บันทึกการเคลื่อนไหวสต็อกยางสำเร็จ! คงเหลือ: {new_quantity} เส้น', 'success')
                cache.clear()
            ...
            # --- Process Wheel Movement ---
                ...
                if move_type == 'IN' or move_type == 'RETURN':
                    ...
                    if new_quantity < quantity_change:
                        flash(
                            f'สต็อกแม็กไม่พอสำหรับการจ่ายออก. มีเพียง {new_quantity} วง.', 'danger')
                        return redirect(url_for('stock_movement', tab=active_tab_on_error))
                    new_quantity -= quantity_change
                ...
                database.add_wheel_movement(conn, wheel_id, move_type, quantity_change, new_quantity, notes,
                                            ...
                                            return_customer_type=return_customer_type)
                flash(
                    f'บันทึกการเคลื่อนไหวสต็อกแม็กสำเร็จ! คงเหลือ: {new_quantity} วง', 'success')
                cache.clear()
            ...
            flash(
                f'เกิดข้อผิดพลาดในการบันทึกการเคลื่อนไหวสต็อก: {e}', 'danger')
            return redirect(url_for('stock_movement', tab=active_tab_on_error))
...
def edit_tire_movement(movement_id):
    ...
    if request.method == 'POST':
        ...
        bill_image_file = request.files.get('bill_image')
        delete_existing_image = request.form.get(
            'delete_existing_image') == 'on'
            ...
            if current_image_url and "res.cloudinary.com" in current_image_url:
                ...
                if public_id_match:
                    ...
                    try:
                        ...
                        print(
                            f"Error deleting old tire movement image from Cloudinary: {e}")
            bill_image_url_to_db = None
        ...
        new_online_platform_id_str = request.form.get(
            'online_platform_id')  # สำหรับ OUT ช่องทาง 'ออนไลน์'
        new_wholesale_customer_id_str = request.form.get(
            'wholesale_customer_id')  # สำหรับ OUT ช่องทาง 'ค้าส่ง'
        new_return_customer_type = request.form.get('return_customer_type')
        # NEW: รับค่าสำหรับ 'ชื่อร้านยางที่คืน'
        new_return_wholesale_customer_id_str = request.form.get(
            'return_wholesale_customer_id')
        # NEW: รับค่าสำหรับ 'แพลตฟอร์มออนไลน์ที่คืน'
        new_return_online_platform_id_str = request.form.get(
            'return_online_platform_id')

        final_new_channel_id = int(
            new_channel_id_str) if new_channel_id_str else None
        ...
        if new_type == 'IN':
            ...
            if new_return_customer_type == 'ออนไลน์':
                if not new_return_online_platform_id_str:  # ใช้ new_return_online_platform_id_str ที่รับมาใหม่
                    ...
                    return render_template('edit_tire_movement.html', movement=movement_data, current_user=current_user, sales_channels=sales_channels, online_platforms=online_platforms, wholesale_customers=wholesale_customers)
                try:
                    final_new_online_platform_id = int(
                        new_return_online_platform_id_str)
                except ValueError:
                    flash('ข้อมูลแพลตฟอร์มออนไลน์ที่คืนไม่ถูกต้อง', 'danger')
                    return render_template('edit_tire_movement.html', movement=movement_data, current_user=current_user, sales_channels=sales_channels, online_platforms=online_platforms, wholesale_customers=wholesale_customers)
            else:
            ...
            if new_return_customer_type == 'หน้าร้านร้านยาง':
                if not new_return_wholesale_customer_id_str:  # ตรวจสอบว่ามีค่าส่งมาหรือไม่
                    ...
                    return render_template('edit_tire_movement.html', movement=movement_data, current_user=current_user, sales_channels=sales_channels, online_platforms=online_platforms, wholesale_customers=wholesale_customers)
                try:
                    final_new_wholesale_customer_id = int(
                        new_return_wholesale_customer_id_str)
                except ValueError:
                    flash('ข้อมูลชื่อร้านยางไม่ถูกต้อง', 'danger')
                    return render_template('edit_tire_movement.html', movement=movement_data, current_user=current_user, sales_channels=sales_channels, online_platforms=online_platforms, wholesale_customers=wholesale_customers)
            ...
            if current_image_url and "res.cloudinary.com" in current_image_url:
                ...
                if public_id_match:
                    ...
                    try:
                        ...
                        print(
                            f"Error deleting old wheel movement image from Cloudinary: {e}")
            bill_image_url_to_db = None
...
def delete_tire_movement_action(movement_id):
    ...
    try:
        ...
        flash(
            f'เกิดข้อผิดพลาดในการลบข้อมูลการเคลื่อนไหวสต็อกยาง: {e}', 'danger')
...
def delete_wheel_movement_action(movement_id):
    ...
    try:
        ...
        flash(
            f'เกิดข้อผิดพลาดในการลบข้อมูลการเคลื่อนไหวสต็อกแม็ก: {e}', 'danger')
...
def summary_details():
    ...
    try:
        start_date_obj = BKK_TZ.localize(datetime.strptime(start_date_str, '%Y-%m-%d')).replace(hour=0, minute=0, second=0,
                                                                                                microsecond=0) if start_date_str else get_bkk_time().replace(hour=0, minute=0, second=0, microsecond=0)
        end_date_obj = BKK_TZ.localize(datetime.strptime(end_date_str, '%Y-%m-%d')).replace(hour=23, minute=59, second=59,
                                                                                            microsecond=999999) if end_date_str else get_bkk_time().replace(hour=23, minute=59, second=59, microsecond=999999)
...
def daily_stock_report():
    ...
    if report_date_str:
        try:
            report_datetime_obj = BKK_TZ.localize(datetime.strptime(
                report_date_str, '%Y-%m-%d')).replace(hour=0, minute=0, second=0, microsecond=0)
            display_date_str = report_datetime_obj.strftime('%d %b %Y')
        except ValueError:
            flash("รูปแบบวันที่ไม่ถูกต้อง กรุณาใช้YYYY-MM-DD", "danger")
            report_datetime_obj = get_bkk_time().replace(
    ...
    sql_date_filter_end_of_day = report_datetime_obj.replace(
    ...
    if is_psycopg2_conn:
        ...
        tire_movements_raw_today = conn.execute(
            tire_movements_query_today, (sql_date_filter,)).fetchall()

    processed_tire_movements_raw_today = []
    for movement in tire_movements_raw_today:
        movement_data = dict(movement)
        movement_data['timestamp'] = convert_to_bkk_time(
            movement_data['timestamp'])
    ...
    day_before_report = report_datetime_obj.replace(
        hour=0, minute=0, second=0) - timedelta(microseconds=1)
    ...
    """
    if is_psycopg2_conn:
        with conn.cursor() as cursor:  # New cursor for this query
            execute(distinct_tire_ids_query_all_history,
                    (sql_date_filter_end_of_day,))
            rows = cursor.fetchall()
            cursor.close()
    else:
        rows = conn.execute(distinct_tire_ids_query_all_history,
                            (sql_date_filter_end_of_day,)).fetchall()
    ...
    if is_psycopg2_conn:
        ...
        initial_quantities_rows = conn.execute(
            query_sqlite, tuple(params)).fetchall()
    ...
    detailed_tire_report = defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0, 'remaining_quantity': 0,
                                       'tire_main_id': None, 'brand': '', 'model': '', 'size': '', 'movements': []})
    ...
    for movement in tire_movements_raw:
        ...
        if movement['type'] == 'IN':
            ...
            # สะสมยอดรับคืน
            detailed_tire_report[key]['RETURN'] += movement['quantity_change']
            # รับคืนเพิ่มสต็อก
            detailed_tire_report[key]['remaining_quantity'] += movement['quantity_change']
    ...
    for tire_id, qty in tire_quantities_before_report.items():
        if not any(item['tire_main_id'] == tire_id for item in tire_movements_raw):
            ...
            if tire_info and not tire_info['is_deleted']:
                key = (tire_info['brand'],
                       tire_info['model'], tire_info['size'])
                if key not in detailed_tire_report:
                    detailed_tire_report[key]['tire_main_id'] = tire_id
    ...
    tire_brand_summaries = defaultdict(
        lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0, 'current_quantity_sum': 0})
    sorted_unique_tire_items = sorted(
        detailed_tire_report.items(), key=lambda x: x[0])
    ...
    for (brand, model, size), data in sorted_unique_tire_items:
        ...
        sorted_detailed_tire_report.append({
            ...
            # Pass individual movements for detail view
    ...
    if is_psycopg2_conn:
        ...
        wheel_movements_raw_today = conn.execute(
            wheel_movements_query_today, (sql_date_filter,)).fetchall()
    ...
    if is_psycopg2_conn:
        cursor_wheel = conn.cursor()
        cursor_wheel.execute(
            distinct_wheel_ids_query_all_history, (sql_date_filter_end_of_day,))
        rows = cursor_wheel.fetchall()
        cursor_wheel.close()
    else:
        rows = conn.execute(distinct_wheel_ids_query_all_history,
                            (sql_date_filter_end_of_day,)).fetchall()
    ...
    if wheel_ids_involved:
        ...
        for row in initial_quantities_rows:
            wheel_quantities_before_report[row['wheel_id']
    ...
    detailed_wheel_report = defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0, 'remaining_quantity': 0,
                                        'wheel_main_id': None, 'brand': '', 'model': '', 'diameter': None, 'pcd': '', 'width': None, 'movements': []})

    for movement in wheel_movements_raw:
        key = (movement['brand'], movement['model'],
               movement['diameter'], movement['pcd'], movement['width'])
        ...
        if movement['type'] == 'IN':
            ...
            # สะสมยอดรับคืน
            detailed_wheel_report[key]['RETURN'] += movement['quantity_change']
            # รับคืนเพิ่มสต็อก
            detailed_wheel_report[key]['remaining_quantity'] += movement['quantity_change']
    ...
    for wheel_id, qty in wheel_quantities_before_report.items():
        if not any(item['wheel_main_id'] == wheel_id for item in wheel_movements_raw):
            ...
            if wheel_info and not wheel_info['is_deleted']:
                key = (wheel_info['brand'], wheel_info['model'],
                       wheel_info['diameter'], wheel_info['pcd'], wheel_info['width'])
                if key not in detailed_wheel_report:
                    detailed_wheel_report[key]['wheel_main_id'] = wheel_id
    ...
    wheel_brand_summaries = defaultdict(
        lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0, 'current_quantity_sum': 0})
    sorted_unique_wheel_items = sorted(
        detailed_wheel_report.items(), key=lambda x: x[0])
    ...
    tire_total_in = sum(
        item['IN'] for item in sorted_detailed_tire_report if not item['is_summary'])
    tire_total_out = sum(
        item['OUT'] for item in sorted_detailed_tire_report if not item['is_summary'])
    tire_total_return = sum(
        item['RETURN'] for item in sorted_detailed_tire_report if not item['is_summary'])
    ...
    if is_psycopg2_conn:
        with conn.cursor() as cursor:
            cursor.execute(query_total_before_tires,
                           (start_of_report_day_iso,))
            initial_total_tires = cursor.fetchone()[0] or 0
            cursor.close()
    else:
        initial_total_tires = conn.execute(
            query_total_before_tires, (start_of_report_day_iso,)).fetchone()[0] or 0

    tire_total_remaining_for_report_date = initial_total_tires + \
        tire_total_in + tire_total_return - tire_total_out

    wheel_total_in = sum(
        item['IN'] for item in sorted_detailed_wheel_report if not item['is_summary'])
    wheel_total_out = sum(
        item['OUT'] for item in sorted_detailed_wheel_report if not item['is_summary'])
    wheel_total_return = sum(
        item['RETURN'] for item in sorted_detailed_wheel_report if not item['is_summary'])
    ...
    if is_psycopg2_conn:
        with conn.cursor() as cursor:
            cursor.execute(query_total_before_wheels,
                           (start_of_report_day_iso,))
            initial_total_wheels = cursor.fetchone()[0] or 0
            cursor.close()
    else:
        initial_total_wheels = conn.execute(
            query_total_before_wheels, (start_of_report_day_iso,)).fetchone()[0] or 0

    wheel_total_remaining_for_report_date = initial_total_wheels + \
        wheel_total_in + wheel_total_return - wheel_total_out
    ...
    return render_template('daily_stock_report.html',
                           ...
                           yesterday_date_param=yesterday_date_calc.strftime(
                               '%Y-%m-%d'),
                           tomorrow_date_param=tomorrow_date_calc.strftime(
                               '%Y-%m-%d'),
                           ...
                           # Pass raw movements for detailed view per day
...
def summary_stock_report():
    ...
    if not start_date_str or not end_date_str:
        ...
        start_date_obj = bkk_tz.localize(datetime(
            first_day_of_month.year, first_day_of_month.month, first_day_of_month.day, 0, 0, 0))
        end_date_obj = bkk_tz.localize(
            datetime(today.year, today.month, today.day, 23, 59, 59, 999999))
        display_range_str = f"จากวันที่ {start_date_obj.strftime('%d %b %Y')} ถึงวันที่ {end_date_obj.strftime('%d %b %Y')}"
    else:
        try:
            start_date_obj = bkk_tz.localize(datetime.strptime(
                start_date_str, '%Y-%m-%d')).replace(hour=0, minute=0, second=0, microsecond=0)
            end_date_obj = bkk_tz.localize(datetime.strptime(
                end_date_str, '%Y-%m-%d')).replace(hour=23, minute=59, second=59, microsecond=999999)
    ...
    tire_movements_by_channel_data = defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': [], 'online_platforms': defaultdict(
        lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0}), 'wholesale_customers': defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0})})
    wheel_movements_by_channel_data = defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': [], 'online_platforms': defaultdict(
        lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0}), 'wholesale_customers': defaultdict(lambda: {'IN': 0, 'OUT': 0, 'RETURN': 0})})
    ...
    try:
        if is_psycopg2_conn:
            with conn.cursor() as cursor:
                cursor.execute(tire_channel_summary_query,
                               tire_channel_summary_params)
            tire_movements_raw_detailed = cursor.fetchall()
            cursor.close()
        else:
            query_for_sqlite = tire_channel_summary_query.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            tire_movements_raw_detailed = conn.execute(
                query_for_sqlite, tire_channel_summary_params).fetchall()
            ...
            if main_channel_key == 'ออนไลน์':
                if online_platform_name and online_platform_name != 'ไม่ระบุแพลตฟอร์ม':
                    if online_platform_name not in tire_movements_by_channel_data[main_channel_key]['online_platforms']:
                        tire_movements_by_channel_data[main_channel_key]['online_platforms'][online_platform_name] = {
                            'IN': 0, 'OUT': 0, 'RETURN': 0}
                    tire_movements_by_channel_data[main_channel_key][
                        'online_platforms'][online_platform_name][move_type] += total_qty
                    tire_movements_by_channel_data[main_channel_key]['online_platforms'][
                        online_platform_name]['id'] = online_platform_id_from_db
                ...
                if wholesale_customer_name and wholesale_customer_name != 'ไม่ระบุลูกค้า':
                    if wholesale_customer_name not in tire_movements_by_channel_data[main_channel_key]['wholesale_customers']:
                        tire_movements_by_channel_data[main_channel_key]['wholesale_customers'][wholesale_customer_name] = {
                            'IN': 0, 'OUT': 0, 'RETURN': 0}
                    tire_movements_by_channel_data[main_channel_key]['wholesale_customers'][
                        wholesale_customer_name][move_type] += total_qty
                    tire_movements_by_channel_data[main_channel_key]['wholesale_customers'][
                        wholesale_customer_name]['id'] = wholesale_customer_id_from_db

        # โค้ด 2 บรรทัดนี้ต้องอยู่ "นอก" for loop แต่ยังอยู่ "ใน" try
        sorted_tire_movements_by_channel = OrderedDict(
            sorted(tire_movements_by_channel_data.items()))

        for channel_name_sort, data_sort in sorted_tire_movements_by_channel.items():
            if 'online_platforms' in data_sort:
                data_sort['online_platforms'] = OrderedDict(
                    sorted(data_sort['online_platforms'].items()))
            if 'wholesale_customers' in data_sort:
                data_sort['wholesale_customers'] = OrderedDict(
                    sorted(data_sort['wholesale_customers'].items()))

    except Exception as e:
        print(
            f"ERROR: Failed to fetch detailed tire movements for summary (Channel): {e}")
    ...
    try:
        if is_psycopg2_conn:
            with conn.cursor() as cursor:
                cursor.execute(wheel_channel_summary_query,
                               wheel_channel_summary_params)
            wheel_movements_raw_detailed = cursor.fetchall()
            cursor.close()
        else:
            query_for_sqlite = wheel_channel_summary_query.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            wheel_movements_raw_detailed = conn.execute(
                query_for_sqlite, wheel_channel_summary_params).fetchall()
        ...
        for movement_row in wheel_movements_raw_detailed:
            ...
            if main_channel_key == 'ออนไลน์':
                if online_platform_name and online_platform_name != 'ไม่ระบุแพลตฟอร์ม':
                    wheel_movements_by_channel_data[main_channel_key][
                        'online_platforms'][online_platform_name][move_type] += total_qty
            elif main_channel_key == 'ค้าส่ง':
                if wholesale_customer_name and wholesale_customer_name != 'ไม่ระบุลูกค้า':
                    wheel_movements_by_channel_data[main_channel_key]['wholesale_customers'][
                        wholesale_customer_name][move_type] += total_qty

        sorted_wheel_movements_by_channel = OrderedDict(
            sorted(wheel_movements_by_channel_data.items()))
        ...
        print(
            f"ERROR: Failed to fetch detailed wheel movements for summary (Channel): {e}")
    ...
    overall_tire_in_period = int(
        sum(data.get('IN', 0) for data in tire_movements_by_channel_data.values()))
    overall_tire_out_period = int(
        sum(data.get('OUT', 0) for data in tire_movements_by_channel_data.values()))
    ...
    overall_tire_return_period = int(sum(
        ...
        # Get the list, default to empty list if not present
        for item in data.get('RETURN', [])
    ))

    overall_wheel_in_period = int(
        sum(data.get('IN', 0) for data in wheel_movements_by_channel_data.values()))
    overall_wheel_out_period = int(
        sum(data.get('OUT', 0) for data in wheel_movements_by_channel_data.values()))
    # Correct sum for RETURN: iterate through the list of dictionaries and sum 'quantity'
    ...
    try:
        ...
        if is_psycopg2_conn:
            with conn.cursor() as cursor:
                cursor.execute(query_overall_initial_tires,
                               (start_of_period_iso,))
            overall_tire_initial = int(cursor.fetchone()[0] or 0)
            cursor.close()
        else:
            query_for_sqlite = query_overall_initial_tires.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            overall_tire_initial = int(conn.execute(
                query_for_sqlite, (start_of_period_iso,)).fetchone()[0] or 0)
    ...
    try:
        ...
        if is_psycopg2_conn:
            with conn.cursor() as cursor:
                cursor.execute(query_overall_initial_wheels,
                               (start_of_period_iso,))
            overall_wheel_initial = int(cursor.fetchone()[0] or 0)
            cursor.close()
        else:
            query_for_sqlite = query_overall_initial_wheels.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            overall_wheel_initial = int(conn.execute(
                query_for_sqlite, (start_of_period_iso,)).fetchone()[0] or 0)
    ...
    overall_tire_final = overall_tire_initial + overall_tire_in_period + \
        overall_tire_return_period - overall_tire_out_period
    overall_wheel_final = overall_wheel_initial + overall_wheel_in_period + \
        overall_wheel_return_period - overall_wheel_out_period
    ...
    try:
        ...
        tire_item_params = (
            ...
            # RETURN_qty sum (param 5,6)
            start_of_period_iso, end_of_period_iso,
            # initial_qty_before_period subquery (param 7)
            start_of_period_iso,
            # HAVING: Any movement in period (param 8,9)
            start_of_period_iso, end_of_period_iso,
            # HAVING: Had initial stock before period (param 10)
            start_of_period_iso
        ...
        if is_psycopg2_conn:
            ...
            query_for_sqlite = tire_detailed_item_query.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            tires_detailed_movements_raw = conn.execute(
                query_for_sqlite, tire_item_params).fetchall()
        ...
        for row_data_raw in tires_detailed_movements_raw:
            ...
            initial_qty = int(normalized_row.get(
                'initial_qty_before_period', 0))
    ...
    try:
        ...
        if is_psycopg2_conn:
            ...
            query_for_sqlite = wheel_detailed_item_query.replace(
                f"{timestamp_cast}", "").replace(placeholder, '?')
            wheels_detailed_movements_raw = conn.execute(
                query_for_sqlite, wheel_item_params).fetchall()
    ...
    try:
        ...
        if is_psycopg2_conn:
            ...
            all_tire_brands = [row['brand']
                               for row in conn.execute(brands_query).fetchall()]
        ...
        for brand in all_tire_brands:
            ...
            if is_psycopg2_conn:
                with conn.cursor() as cursor:
                    cursor.execute(query_brand_initial_tire,
                                   (brand, start_of_period_iso))
                brand_initial_qty = int(cursor.fetchone()[0] or 0)
                cursor.close()
            else:
                query_for_sqlite = query_brand_initial_tire.replace(
                    f"{timestamp_cast}", "").replace(placeholder, '?')
                brand_initial_qty = int(conn.execute(
                    query_for_sqlite, (brand, start_of_period_iso)).fetchone()[0] or 0)
            ...
            final_qty_brand = brand_initial_qty + total_in_brand + \
                total_return_brand - total_out_brand
    ...
    try:
        ...
        for brand in all_wheel_brands:
            ...
            if is_psycopg2_conn:
                with conn.cursor() as cursor:
                    cursor.execute(query_brand_initial_wheel,
                                   (brand, start_of_period_iso))
                brand_initial_qty = int(cursor.fetchone()[0] or 0)
                cursor.close()
            else:
                query_for_sqlite = query_brand_initial_wheel.replace(
                    f"{timestamp_cast}", "").replace(placeholder, '?')
                brand_initial_qty = int(conn.execute(
                    query_brand_initial_wheel, (brand, start_of_period_iso)).fetchone()[0] or 0)
    ...
    for brand, items in tires_by_brand_for_summary_report.items():
        ...
        moved_items = [item for item in items if item['IN']
                       > 0 or item['OUT'] > 0 or item['RETURN'] > 0]
    ...
    return render_template('summary_stock_report.html',
                           start_date_param=start_date_obj.strftime(
                               '%Y-%m-%d'),
    ...
    if file and allowed_excel_file(file.filename):
        try:
            ...
            if not all(col in df.columns for col in expected_tire_cols):
                missing_cols = [
                    col for col in expected_tire_cols if col not in df.columns]
                flash(
                    f'ไฟล์ Excel ขาดคอลัมน์ที่จำเป็น: {", ".join(missing_cols)}. โปรดดาวน์โหลดไฟล์ตัวอย่างเพื่อดูรูปแบบที่ถูกต้อง.', 'danger')
                return redirect(url_for('export_import', tab='tires_excel'))
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    barcode_id_from_excel = str(
                        row.get('Barcode ID', '')).strip()
                    ...
                    if not brand or not model or not size:
                        raise ValueError(
                            "ข้อมูล 'ยี่ห้อ', 'รุ่นยาง', หรือ 'เบอร์ยาง' ไม่สามารถเว้นว่างได้")

                    quantity = int(row['สต็อก']) if pd.notna(
                        row['สต็อก']) else 0
                    price_per_item = float(row['ราคาต่อเส้น']) if pd.notna(
                        row['ราคาต่อเส้น']) else 0.0
                    ...
                    cost_sc = float(cost_sc_raw) if pd.notna(
                        cost_sc_raw) else None
                    cost_dunlop = float(cost_dunlop_raw) if pd.notna(
                        cost_dunlop_raw) else None
                    cost_online = float(cost_online_raw) if pd.notna(
                        cost_online_raw) else None
                    wholesale_price1 = float(wholesale_price1_raw) if pd.notna(
                        wholesale_price1_raw) else None
                    wholesale_price2 = float(wholesale_price2_raw) if pd.notna(
                        wholesale_price2_raw) else None
                    ...
                    if pd.notna(year_of_manufacture_raw):
                        try:
                            ...
                            year_of_manufacture = str(
                                year_of_manufacture_raw).strip()
                            if year_of_manufacture == 'nan':
                                year_of_manufacture = None

                    promotion_id = int(row.get('ID โปรโมชัน')) if pd.notna(
                        row.get('ID โปรโมชัน')) else None
                    ...
                    if barcode_id_to_save:
                        existing_tire_id_by_barcode = database.get_tire_id_by_barcode(
                            conn, barcode_id_to_save)
                        if existing_tire_id_by_barcode:
                            if "psycopg2" in str(type(conn)):
                                cursor.execute(
                                    "SELECT id, brand, model, size, quantity FROM tires WHERE id = %s", (existing_tire_id_by_barcode,))
                            else:
                                cursor.execute(
                                    "SELECT id, brand, model, size, quantity FROM tires WHERE id = ?", (existing_tire_id_by_barcode,))
                        ...
                        if existing_wheel_id_by_barcode:
                            raise ValueError(
                                f"Barcode ID '{barcode_id_to_save}' ซ้ำกับล้อแม็ก ID {existing_wheel_id_by_barcode}. Barcode ID ต้องไม่ซ้ำกันข้ามประเภทสินค้า.")

                    if not existing_tire:
                        if "psycopg2" in str(type(conn)):
                            cursor.execute(
                                "SELECT id, brand, model, size, quantity FROM tires WHERE brand = %s AND model = %s AND size = %s", (brand, model, size))
                        else:
                            cursor.execute(
                                "SELECT id, brand, model, size, quantity FROM tires WHERE brand = ? AND model = ? AND size = ?", (brand, model, size))
                    ...
                    if existing_tire:
                        ...
                        if barcode_id_to_save and not database.get_tire_id_by_barcode(conn, barcode_id_to_save):
                            database.add_tire_barcode(
                                conn, tire_id, barcode_id_to_save, is_primary=False)
                        ...
                        if quantity != old_quantity:
                            ...
                            database.add_tire_movement(conn, tire_id, movement_type, quantity_change_diff,
                                                       quantity, "Import from Excel (Qty Update)", None, user_id=current_user.id)
                        updated_count += 1
                        ...
                        if barcode_id_to_save:
                            database.add_tire_barcode(
                                conn, new_tire_id, barcode_id_to_save, is_primary=True)
                        database.add_tire_movement(conn, new_tire_id, 'IN', quantity, quantity,
                                                   "Import from Excel (initial stock)", None, user_id=current_user.id)
                        imported_count += 1

                except Exception as row_e:
                    error_rows.append(
                        f"แถวที่ {index + 2}: {row_e} - {row.to_dict()}")
            ...
            flash(
                f'เกิดข้อผิดพลาดร้ายแรงในการนำเข้าไฟล์ Excel ของยาง: {e}', 'danger')
    ...
    if file and allowed_excel_file(file.filename):
        try:
            ...
            if not all(col in df.columns for col in expected_wheel_cols):
                missing_cols = [
                    col for col in expected_wheel_cols if col not in df.columns]
                flash(
                    f'ไฟล์ Excel ขาดคอลัมน์ที่จำเป็น: {", ".join(missing_cols)}. โปรดดาวน์โหลดไฟล์ตัวอย่างเพื่อดูรูปแบบที่ถูกต้อง.', 'danger')
                return redirect(url_for('export_import', tab='wheels_excel'))
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    if not brand or not model or not pcd:
                        raise ValueError(
                            "ข้อมูล 'ยี่ห้อ', 'ลาย', หรือ 'รู' ไม่สามารถเว้นว่างได้")

                    diameter = float(row['ขอบ']) if pd.notna(
                        row['ขอบ']) else 0.0
                    width = float(row['กว้าง']) if pd.notna(
                        row['กว้าง']) else 0.0
                    quantity = int(row['สต็อก']) if pd.notna(
                        row['สต็อก']) else 0
                    cost = float(row['ทุน']) if pd.notna(row['ทุน']) else None
                    retail_price = float(row['ราคาขายปลีก']) if pd.notna(
                        row['ราคาขายปลีก']) else 0.0
                    ...
                    color = str(color_raw).strip() if pd.notna(
                        color_raw) else None
                    image_url = str(image_url_raw).strip(
                    ) if pd.notna(image_url_raw) else None
                    cost_online = float(cost_online_raw) if pd.notna(
                        cost_online_raw) else None
                    wholesale_price1 = float(wholesale_price1_raw) if pd.notna(
                        wholesale_price1_raw) else None
                    ...
                    if barcode_id_to_save:
                        ...
                        if existing_wheel_id_by_barcode:
                            if "psycopg2" in str(type(conn)):
                                cursor.execute(
                                    "SELECT id, brand, model, diameter, pcd, width, quantity FROM wheels WHERE id = %s", (existing_wheel_id_by_barcode,))
                            else:
                                cursor.execute(
                                    "SELECT id, brand, model, diameter, pcd, width, quantity FROM wheels WHERE id = ?", (existing_wheel_id_by_barcode,))
                        ...
                        if existing_tire_id_by_barcode:
                            raise ValueError(
                                f"Barcode ID '{barcode_id_to_save}' ซ้ำกับยาง ID {existing_tire_id_by_barcode}. Barcode ID ต้องไม่ซ้ำกันข้ามประเภทสินค้า.")
                    ...
                    if existing_wheel:
                        ...
                        if barcode_id_to_save and not database.get_wheel_id_by_barcode(conn, barcode_id_to_save):
                            database.add_wheel_barcode(
                                conn, wheel_id, barcode_id_to_save, is_primary=False)

                        database.update_wheel_import(conn, wheel_id, brand, model, diameter, pcd, width, et, color,
                        ...
                        if quantity != old_quantity:
                            movement_type = 'IN' if quantity > old_quantity else 'OUT'
                            quantity_change_diff = abs(quantity - old_quantity)
                            database.add_wheel_movement(conn, wheel_id, movement_type, quantity_change_diff,
                                                        quantity, "Import from Excel (Qty Update)", None, user_id=current_user.id)
                        updated_count += 1

                    else:
                        new_wheel_id = database.add_wheel_import(
                            conn, brand, model, diameter, pcd, width, et, color, quantity, cost, cost_online, wholesale_price1, wholesale_price2, retail_price, image_url)
                        if barcode_id_to_save:
                            database.add_wheel_barcode(
                                conn, new_wheel_id, barcode_id_to_save, is_primary=True)
                        database.add_wheel_movement(
                            conn, new_wheel_id, 'IN', quantity, quantity, "Import from Excel (initial stock)", None, user_id=current_user.id)
                        imported_count += 1
                except Exception as row_e:
                    error_rows.append(
                        f"แถวที่ {index + 2}: {row_e} - {row.to_dict()}")
            ...
            flash(
                f'เกิดข้อผิดพลาดร้ายแรงในการนำเข้าไฟล์ Excel ของแม็ก: {e}', 'danger')
            if 'db' in g and g.db is not None:
                g.db.rollback()
            return redirect(url_for('export_import', tab='wheels_excel'))
    else:
...
def edit_user_role(user_id):
    ...
    allowed_roles = ['admin', 'editor',
                     'retail_sales', 'wholesale_sales', 'viewer']
    ...
    if success:
        flash(
            f'แก้ไขบทบาทผู้ใช้ ID {user_id} เป็น "{new_role}" สำเร็จ!', 'success')
    else:
...
def admin_dashboard():
    ...
    try:
        ...
        if needs_cleanup:
            deleted_count = database.delete_old_activity_logs(
                conn, days=CLEANUP_INTERVAL_DAYS)
            # บันทึกเวลาปัจจุบันลงฐานข้อมูลเป็นการล้างครั้งล่าสุด
            database.set_setting(conn, 'last_log_cleanup',
                                 datetime.now().isoformat())
            conn.commit()  # Commit ทั้งการลบ Log และการอัปเดตค่า setting

            flash(
                f'ล้างประวัติการใช้งานที่เก่ากว่า {CLEANUP_INTERVAL_DAYS} วันเรียบร้อยแล้ว (ลบไป {deleted_count} รายการ)', 'info')
            print(
                f"AUTOMATIC LOG CLEANUP: Deleted {deleted_count} old activity logs.")
...
# Renamed to avoid conflict with barcode_scanner route
@app.route('/barcode_scanner_page')
@login_required
...
def api_process_stock_transaction():
    ...
    try:
        for item_data in items_to_process:
            ...
            if item_type == 'tire':
                database.add_tire_movement(
                    conn, item_id, transaction_type, quantity_change, new_qty, notes, None, user_id)
            elif item_type == 'wheel':
                database.add_wheel_movement(
                    conn, item_id, transaction_type, quantity_change, new_qty, notes, None, user_id)
...
def api_search_items_for_link():
    ...
    """
    if "psycopg2" in str(type(conn)):
        cursor.execute(tire_search_query,
                       (f"%{query}%", f"%{query}%", f"%{query}%"))
        tire_results = cursor.fetchall()
    else:
        tire_results = conn.execute(tire_search_query.replace(
            '%s', '?'), (f"%{query}%", f"%{query}%", f"%{query}%")).fetchall()
    ...
    """
    if "psycopg2" in str(type(conn)):
        cursor.execute(wheel_search_query,
                       (f"%{query}%", f"%{query}%", f"%{query}%"))
        wheel_results = cursor.fetchall()
    else:
        wheel_results = conn.execute(wheel_search_query.replace(
            '%s', '?'), (f"%{query}%", f"%{query}%", f"%{query}%")).fetchall()
...
def api_link_barcode_to_item():
    ...
    conn = get_db()
    try:
        existing_tire_barcode_id = database.get_tire_id_by_barcode(
            conn, scanned_barcode)
        existing_wheel_barcode_id = database.get_wheel_id_by_barcode(
            conn, scanned_barcode)
        ...
        if item_type == 'tire':
            database.add_tire_barcode(
                conn, item_id, scanned_barcode, is_primary=False)
        elif item_type == 'wheel':
            database.add_wheel_barcode(
                conn, item_id, scanned_barcode, is_primary=False)
        else:
...
def add_wholesale_customer_action():
    ...
    if not customer_name:
        ...
        if customer_id:
            ...
            flash(
                f'ไม่สามารถเพิ่มลูกค้าค้าส่ง "{customer_name}" ได้ อาจมีชื่อนี้อยู่ในระบบแล้ว', 'warning')
    return redirect(url_for('manage_wholesale_customers'))
...
def edit_wholesale_customer(customer_id):
    ...
    with conn.cursor() as cursor:
        if "psycopg2" in str(type(conn)):
            cursor.execute(
            "SELECT id, name FROM wholesale_customers WHERE id = %s", (customer_id,))
        else:
            cursor.execute(
            "SELECT id, name FROM wholesale_customers WHERE id = ?", (customer_id,))
    ...
    if request.method == 'POST':
        ...
        if not new_name:
            ...
            try:
                ...
                if "psycopg2" in str(type(conn)):
                    cursor.execute(
                        "UPDATE wholesale_customers SET name = %s WHERE id = %s", (new_name, customer_id))
                else:
                    cursor.execute(
                        "UPDATE wholesale_customers SET name = ? WHERE id = ?", (new_name, customer_id))
                conn.commit()
                flash(
                    f'แก้ไขชื่อลูกค้าค้าส่งเป็น "{new_name}" สำเร็จ!', 'success')
                cache.delete_memoized(get_cached_wholesale_summary)
                return redirect(url_for('manage_wholesale_customers'))
            except Exception as e:
                conn.rollback()
                if "UNIQUE constraint failed" in str(e) or "duplicate key value violates unique constraint" in str(e):
                    flash(
                        f'ชื่อลูกค้าค้าส่ง "{new_name}" มีอยู่ในระบบแล้ว', 'warning')
                else:
                    flash(
                        f'เกิดข้อผิดพลาดในการแก้ไขลูกค้าค้าส่ง: {e}', 'danger')
...
def delete_wholesale_customer(customer_id):
    ...
    try:
        ...
        if is_postgres:
            cursor.execute(
                "UPDATE tire_movements SET wholesale_customer_id = NULL WHERE wholesale_customer_id = %s", (customer_id,))
            cursor.execute(
                "UPDATE wheel_movements SET wholesale_customer_id = NULL WHERE wholesale_customer_id = %s", (customer_id,))
            cursor.execute(
                "DELETE FROM wholesale_customers WHERE id = %s", (customer_id,))
        else:
            cursor.execute(
                "UPDATE tire_movements SET wholesale_customer_id = NULL WHERE wholesale_customer_id = ?", (customer_id,))
            cursor.execute(
                "UPDATE wheel_movements SET wholesale_customer_id = NULL WHERE wholesale_customer_id = ?", (customer_id,))
            cursor.execute(
                "DELETE FROM wholesale_customers WHERE id = ?", (customer_id,))
...
def bulk_stock_movement():
    ...
    try:
        ...
        return_customer_type = request.form.get('return_customer_type')
        return_wholesale_customer_id = request.form.get(
            'return_wholesale_customer_id')
        return_online_platform_id = request.form.get(
            'return_online_platform_id')
        ...
        if move_type == 'RETURN':
            if return_customer_type == 'ออนไลน์':
                final_online_platform_id = int(
                    return_online_platform_id) if return_online_platform_id else None
            elif return_customer_type == 'หน้าร้านร้านยาง':
                final_wholesale_customer_id = int(
                    return_wholesale_customer_id) if return_wholesale_customer_id else None
        elif move_type == 'OUT':
            final_online_platform_id = int(
                online_platform_id) if online_platform_id else None
            final_wholesale_customer_id = int(
                wholesale_customer_id) if wholesale_customer_id else None
        ...
        for item_data in items:
            ...
            if move_type == 'IN' or move_type == 'RETURN':
                ...
                if current_quantity < quantity_change:
                    ...
                    raise ValueError(
                        f"สต็อกไม่พอสำหรับ {item_name} (มี: {current_quantity}, ต้องการ: {quantity_change})")
                new_quantity -= quantity_change
        ...
        current_app.logger.error(
            f"Bulk stock movement failed: {e}", exc_info=True)
...
def update_feedback_status(feedback_id):
    ...
    try:
        ...
        conn.commit()
        flash(
            f'อัปเดตสถานะของ Feedback ID #{feedback_id} เป็น "{new_status}" สำเร็จ!', 'success')
    except Exception as e:
        conn.rollback()
...
def wholesale_customer_detail(customer_id):
    ...
    # วันที่เริ่มต้นคือวันที่ 1 ของเดือนปัจจุบัน
        ...
        try:
            start_date_obj = BKK_TZ.localize(datetime.strptime(
                start_date_str, '%Y-%m-%d')).replace(hour=0, minute=0, second=0)
            end_date_obj = BKK_TZ.localize(datetime.strptime(
    ...
    history = database.get_wholesale_customer_purchase_history(
        conn, customer_id, start_date=start_date_obj, end_date=end_date_obj)
...
def log_activity(response):
    ...
    should_log = (
        request.method in ['POST', 'PUT',
                           'DELETE'] and response.status_code in [200, 201, 302]
    ) or (
